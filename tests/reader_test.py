...
class ParseDateTestCase(TestCase):
    ...
    def test_parse_date(self):
        ...
        assert_equal(reader.parse_date(self.config, date_string), expected)
    ...
    def test_recent_items(self):
        ...
        recent = reader.get_recent_items(self.config, feed, base_date)
...
class GetFeedEntriesTestCase(TestCase):
    ...
    def patch_deps(self):
        ...
        with contextlib.nested(
            mock.patch('rssdigest.reader.feedparser'),
            mock.patch('rssdigest.reader.get_recent_items')
        ) as (self.mock_parser, self.mock_get_recent):
            yield

    def test_get_feed_entries_none(self):
        self.mock_get_recent.return_value = []
        feed = reader.get_feed_entries(self.config, self.min_datetime)
        assert not feed
